from aiogram import Router, types# in
...
async def show_leased(message: types.Message, state: FSMContext) :
    ...
    if len(leased_games) == 0 :
        ...
        for row in leased_games :
            buttons.append([InlineKeyboardButton(text=row[2] + " вернуть " + row[0], callback_data=str(row[-1]))])
    buttons.append([InlineKeyboardButton(text="закрыть", callback_data="close")])
    await state.set_state(States.leased_shown)
    await message.answer(answer, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
